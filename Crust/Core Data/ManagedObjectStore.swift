...
*/
@objc protocol CRManagedObjectCaching : NSObjectProtocol {
    ...
    */
    ...
    func managedObjectsWithEntity(entity: NSEntityDescription, attributeValues: NSDictionary, inManagedObjectContext: NSManagedObjectContext) -> NSSet
    
    ///---------------------------------------------------
    /// @name Handling Managed Object Change Notifications
    ///---------------------------------------------------
    
    optional func didFetchObject(object: NSManagedObject)
    optional func didCreateObject(object: NSManagedObject)
    optional func didDeleteObject(object: NSManagedObject)
}

let RKErrorDomain = "RKErrorDomain"

let RKSQLitePersistentStoreSeedDatabasePathOption = "RKSQLitePersistentStoreSeedDatabasePathOption"
let RKManagedObjectStoreDidFailSaveNotification = "RKManagedObjectStoreDidFailSaveNotification"
let RKManagedObjectStoreDidResetPersistentStoresNotification = "RKManagedObjectStoreDidResetPersistentStoresNotification"

extension NSManagedObjectContext {
    func isDecendentOfContext(potentialAncestor: NSManagedObjectContext) -> Bool {
        while let context = self.parentContext {
            if context.isEqual(potentialAncestor) {
                return true
            }
        }
        return false;
...
extension NSNotification {
    func setOfManagedObjectIDsIfManagedObjectContextDidSaveNotification() -> Set<NSObject> {
        var objectIDs = Set<NSObject>()
        if let objects = self.userInfo?.values {
            for object in objects {
                objectIDs.unionInPlace(object as! Array)   // TODO: Fucking !.
            }
        }
...
class ManagedObjectContextChangeMergingObserver {
    var observedContext: NSManagedObjectContext
    var mergeContext: NSManagedObjectContext
    var objectIdsFromChildDidSaveNotification: Set<NSObject>? = []
    
    let defaultCenter = NSNotificationCenter.defaultCenter()
    
    init(withObservedContext observedContext: NSManagedObjectContext, mergeContext: NSManagedObjectContext) {
        self.observedContext = observedContext;
        self.mergeContext = mergeContext;
        self.objectIdsFromChildDidSaveNotification = Set()
        ...
        if mergeContext.isDecendentOfContext(observedContext) {
            NSLog("Detected observation of ancestor context by child: enabling child context save detection")
    ...
    @objc func handleManagedObjectContextWillSaveNotification(notification: NSNotification) {
        self.objectIdsFromChildDidSaveNotification = notification.setOfManagedObjectIDsIfManagedObjectContextDidSaveNotification()
    }
    
    @objc func handleManagedObjectContextDidSaveNotification(notification: NSNotification) {
        guard let object = notification.object where object as! NSObject == self.observedContext else {
            assert(false, "Received Managed Object Context Did Save Notification for Unexpected Context:\(notification.object)")
        }
        ...
        */
        ...
        let managedObjectIDs = notification.setOfManagedObjectIDsIfManagedObjectContextDidSaveNotification()
        if self.objectIdsFromChildDidSaveNotification != managedObjectIDs {
            self.mergeContext.performBlock {
                if let objects = notification.userInfo?[NSUpdatedObjectsKey] as! Array<NSManagedObject>? {
                    for object in objects {
                        let objectID = object.objectID
                        if !objectID.temporaryID {
                            var updatedObject: NSManagedObject
                            do {
                                updatedObject = try self.mergeContext.existingObjectWithID(objectID)
                                updatedObject.willAccessValueForKey(nil)
                            } catch let error as NSError {
                                NSLog("Failed to get existing object for objectID (\(objectID)). Failed with error: \(error)")
                            }
                        }
                    }
                } else {
                    NSLog("Skipping merge of `NSManagedObjectContextDidSaveNotification`: the save event originated from the mergeContext and thus no save is necessary.")
                }
            }
        }
...
class CRManagedObjectStore {
    var managedObjectModel: NSManagedObjectModel
    var persistentStoreCoordinator: NSPersistentStoreCoordinator
    var persistentStoreManagedObjectContext: NSManagedObjectContext
    var mainQueueManagedObjectContext: NSManagedObjectContext
    var managedObjectCache: CRManagedObjectCaching
    
    static var defaultStore = CRManagedObjectStore()
    
    init(withManagedObjectModel managedObjectModel: NSManagedObjectModel) {
        self.managedObjectModel = managedObjectModel
        self.managedObjectCache = CRFetchRequestManagedObjectCache()
    }
...
var RKManagedObjectContextChangeMergingObserverAssociationKey: Character = "\0"
